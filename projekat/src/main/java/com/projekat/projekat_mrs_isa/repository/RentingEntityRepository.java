...
public interface RentingEntityRepository extends JpaRepository<RentingEntity, Long> {
    ...
    @Query(value = "SELECT cl from Client cl JOIN FETCH cl.subscriptions as sub where sub.id=?1")
    List<Client> getSubscribersByRentingEntityId(Long rentingEntityID);
