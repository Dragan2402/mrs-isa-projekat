  ...
  methods: {
    ...
    saveInfoEdit() {
      ...
      if (this.changingPicture === true) {
        ...
        axios.put("/api/users/loggedUser/picture", formDataPicture, {headers: {"Authorization": `Bearer ${localStorage.getItem("jwt")}`}}).then(response => (this.profilePicture = response.data))
      }
      axios.put("api/admins/loggedAdmin", this.loggedAdmin, {headers: {"Authorization": `Bearer ${localStorage.getItem("jwt")}`}}).then(response => {
