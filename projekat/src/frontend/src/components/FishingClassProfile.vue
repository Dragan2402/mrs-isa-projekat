  ...
  <div class="main-container">
    <div class="heading-flex">
      <div class="heading-left">
        <h3 class="main-heading">{{fishingClass.name}}</h3>
        <vue3-star-ratings class="star-ratings" v-model="fishingClass.rating" starSize="22"  :showControl=false :disableClick=true :step=0 />
        <h5 class="star-heading">({{fishingClass.reviewsNumber}})</h5>
      </div>
    ...
    <div class="main-description">
      <div class="inner-description">
        ...
        <div><b>Description: </b> {{fishingClass.promoDescription}}</div>
        <div><b>Behaviour rules: </b> {{fishingClass.behaviourRules}}</div>
        <div><b>Price: </b> {{fishingClass.priceList}} per day</div>
        <div><b>Additional info: </b>{{fishingClass.additionalInfo}}</div>
        <div><b>Cancellation conditions: </b>{{fishingClass.cancellationConditions}}</div>
        <div><b>Instructor Biography: </b>{{fishingClass.instructorBiography}}</div>
        <div><b>Client Limit: </b>{{fishingClass.clientLimit}}</div>
      </div>
      <div class="google-map-container">
        <iframe class="google-map" v-bind:src="'https://maps.google.com/maps?q=' + fishingClass.address + '&t=&z=13&ie=UTF8&iwloc=&output=embed'"></iframe>
      </div>
    </div>
    <hr>
    <div v-if="fishingClass.availableFrom != null && fishingClass.availableFrom != null" v >
      ...
      <div class="modal-content">
        <div class="modal-body">
          <div class="form-floating mb-3">
            <input v-model="fishingClass.name" type="text" class="form-control" id="floatName" placeholder="Title">
            <label for="floatName">Title</label>
          </div>
          <div class="form-floating mb-3">
            <input v-model="fishingClass.address" type="text" class="form-control" id="floatAddress" placeholder="Address">
            <label for="floatAddress">Address</label>
          </div>
          <div class="form-floating mb-3">
            <textarea style="height: 100px" v-model="fishingClass.promoDescription" type="text" class="form-control" id="floatPromoDescription" placeholder="Description"></textarea>
            <label for="floatPromoDescription">Description</label>
          </div>
          <div class="form-floating mb-3">
            <textarea style="height: 100px" v-model="fishingClass.behaviourRules" type="text" class="form-control" id="floatBehaviourRules" placeholder="Behaviour rules"></textarea>
            <label for="floatBehaviourRules">Behaviour rules</label>
          </div>
          <div class="form-floating mb-3">
            <input v-model="fishingClass.priceList" type="text" class="form-control" id="floatPriceList" placeholder="Price">
            <label for="floatPriceList">Price</label>
          </div>
          <div class="form-floating mb-3">
            <textarea style="height: 100px" v-model="fishingClass.additionalInfo" type="text" class="form-control" id="floatAdditionalInfo" placeholder="Additional info"></textarea>
            <label for="floatAdditionalInfo">Additional info</label>
          </div>
          <div class="form-floating mb-3">
            <input v-model="fishingClass.cancellationConditions" type="text" class="form-control" id="floatCancellationConditions" placeholder="Cancellation conditions">
            <label for="floatCancellationConditions">Cancellation conditions</label>
          </div>
          <div class="form-floating mb-3">
            <input v-model="fishingClass.instructorBiography" type="text" class="form-control" id="floatInstructorBiography" placeholder="Instructor Biography">
            <label for="floatInstructorBiography">Instructor Biography</label>
          </div>
          <div class="form-floating mb-3">
            <input v-model="fishingClass.clientLimit" type="text" class="form-control" id="floatClientLimitClass" placeholder="Client Limit">
            <label for="floatClientLimitClass">Client Limit</label>
          </div>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      axios
          .get(`/api/fishingClasses/anyUser/${id.value}`, {headers: {"Authorization" : `Bearer ${localStorage.getItem("jwt")}`}})
          .then(response => {
            fishingClass.value = response.data;
            let from = stringToDateTime(fishingClass.value.availableFrom);
            let to = stringToDateTime(fishingClass.value.availableTo);
      ...
      axios
          .get(`/api/fishingClasses/anyUser/${id.value}/pictures/all`, {headers: {"Authorization" : `Bearer ${localStorage.getItem("jwt")}`}})
          .then(response => {
    ...
    function saveEdit(){
      if(availabilityInterval.value.start != undefined && availabilityInterval.value.end != undefined) {
        fishingClass.value.availableFrom = dateTimeToString(availabilityInterval.value.start);
        fishingClass.value.availableTo = dateTimeToString(availabilityInterval.value.end);
        ...
        axios
            .put(`/api/fishingClasses/loggedfishingClassOwner/${id.value}`, fishingClass.value, { headers: {"Authorization" : `Bearer ${localStorage.getItem("jwt")}`} })
            .then(response => {
              fishingClass.value = response.data;
